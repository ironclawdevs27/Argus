...
landing/
  ...
  index.html                  — Vite entry; favicon link
...
ARGUS_WATCH_INTERVAL_MS=           # watch mode poll interval ms (default: 3000)
...
**Landing page** (`landing/`): React 18 + Vite + Tailwind + Framer Motion SPA with Supabase-backed waitlist and enterprise contact forms. Deployed to `argus-qa.com` via Cloudflare Pages; video served from Cloudflare R2. Sprint 0 mobile fixes applied (2026-05-26): 44px touch targets, `MotionConfig reducedMotion="user"`, modal `100dvh` keyboard fix, `@supports (height: 100dvh)` CSS. SEO: OG tags, Twitter card, canonical, JSON-LD, `robots.txt`, `sitemap.xml`. Video poster: `landing/public/argus-poster.png` (`Argus_bg.png`); `poster="/argus-poster.png"` on `<video>` tag.
