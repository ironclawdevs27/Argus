...
128 test blocks · 544 hard assertions · 49 verified detection categories · 55 fixture pages

> **Coverage note**: 56+ detection categories exist in production code. 49 are positively exercised by the harness (blocks [1]–[93] + [127]–[128]). Blocks [94]–[126] cover infrastructure contracts (module APIs, MCP transport, unhappy paths, CLI E2E). Block [127] adds A7 Theme & Dark Mode; block [128] adds D9 Design Fidelity. Remaining untriggered detections tracked in [argus-v6-strategy.md §10](../argus-v6-strategy.md).
...
| 128 | `design-fidelity.html` | D9 Design Fidelity — `analyzeDesignFidelity` returns array · ≥2 `design_token_mismatch` warnings · all mismatch severity is warning · `--color-primary` mismatch detected · `design_component_missing` for `.figma-hero-section` · `design_fidelity_summary` present · `parseFigmaUrl` extracts fileKey · `parseFigmaUrl` normalises nodeId · `parseFigmaUrl` returns null for invalid URL | Hard |
...
**Expected output (541/544 — 3 permanent MCP-limited failures, exit code 0):**
...
────────────────────────────────────────────────────────
Results: 541/544 hard assertions passed, 3 failed
