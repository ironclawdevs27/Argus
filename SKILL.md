...
- `test-harness/validate.js` — 168-block correctness harness (978 hard assertions)
...
| Metric | Value |
| --- | --- |
| **Version** | `9.9.0` (**Aegis** confidentiality egress boundary — default-ON secret/PII redaction at all five sinks, fail-closed, deny-by-default, OWASP LLM02:2025; also ships the full PR-Validator Phase A–F batch + the landing Security & Compliance section; published to npm, `action.yml` pinned to 9.9.0) |
| **Test blocks** | 168 |
| **Hard assertions** | 978 |
| **Soft assertions** | ~23 executed/run (Lighthouse / perf-trace / heap; Lighthouse now runs headless, the rest are environment-sensitive). Promoted to counted hard assertions in the weekly strict-soft lane via `ARGUS_HARNESS_STRICT_SOFT` (Phase 4.3 — `harness-strict.yml`); clean lane run = 1001/1001 (978 + 23; Linux-headless lane — projected, verified there not on Windows) |
| **Unit tests** | 495 (24 Vitest files, no Chrome — `npm run test:unit`; +129 from Aegis: secret-patterns / sensitivity-classifier / aegis-redaction.fuzz / aegis-redteam — engine Steps 1–2 + Step-4 egress helpers — plus aegis-sinks.test.js for the Steps 5–7 Slack/GitHub/HTML/CI sink guards, and aegis-vault.test.js for the Step-8 reversible re-hydration vault) |
...
| **Harness gate** | **978/978** (no permanent failures — exits 0) |
