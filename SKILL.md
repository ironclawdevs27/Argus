...
| Metric | Value |
| --- | --- |
| **Version** | `9.9.0` published (**Aegis** confidentiality egress boundary — default-ON secret/PII redaction at all five sinks, fail-closed, deny-by-default, OWASP LLM02:2025; ships the full PR-Validator Phase A–F batch + the landing Security & Compliance section; published to npm, `action.yml` pinned to 9.9.0). **In-tree, pending ONE OSS release:** the Aegis-for-Teams engine `policy` param (`redaction-policy.js` + threading + block [170]), the governance seam (`governance-seam.js` — opt-in Ed25519 fetch/verify + secret-free aggregate POST + block [171]), **and the §9 team-vault routing** (`team-vault.js` — opt-in `mode=token`→central vault + block [172], `proof:gov-vault` 13/13) — the version bumps on that publish |
| **Test blocks** | 171 |
| **Hard assertions** | 998 |
