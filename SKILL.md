...
- `test-harness/validate.js` — 171-block correctness harness (998 hard assertions)
...
| Metric | Value |
| --- | --- |
| **Version** | `10.0.0` published (**Aegis for Teams** — opt-in org governance atop the v9.9.0 **Aegis** confidentiality egress boundary — default-ON secret/PII redaction at all five sinks, fail-closed, deny-by-default, OWASP LLM02:2025; ships the full PR-Validator Phase A–F batch + the landing Security & Compliance section; published to npm, `action.yml` pinned to 10.0.0). **Aegis-for-Teams — released in v10.0.0:** the Aegis-for-Teams engine `policy` param (`redaction-policy.js` + threading + block [170]), the governance seam (`governance-seam.js` — opt-in Ed25519 fetch/verify + secret-free aggregate POST + block [171]), **and the §9 team-vault routing** (`team-vault.js` — opt-in `mode=token`→central vault + block [172], `proof:gov-vault` 13/13) — all opt-in + default byte-identical, no new dependency, released in v10.0.0 |
| **Test blocks** | 171 |
| **Hard assertions** | 998 |
| **Soft assertions** | ~23 executed/run (Lighthouse / perf-trace / heap; Lighthouse now runs headless, the rest are environment-sensitive). Promoted to counted hard assertions in the weekly strict-soft lane via `ARGUS_HARNESS_STRICT_SOFT` (Phase 4.3 — `harness-strict.yml`); clean lane run = 1021/1021 (998 + 23; Linux-headless lane — projected, verified there not on Windows) |
| **Unit tests** | 562 (27 Vitest files, no Chrome — `npm run test:unit`; +129 from Aegis Steps 1–8, +32 from the Aegis-for-Teams engine `policy` param, +21 from the governance seam — `governance-seam.test.js`, +14 from the §9 team-vault routing — `team-vault.test.js`: opt-in activation, mint+buffer+dedup, flush posts bearer-authed + clears + scrubs artifactRef, fail-closed on throw/HTTP-error, opt-in default-identical, team>local precedence) |
...
| **Harness gate** | **998/998** on a clean environment / the Linux CI lane (992 prior + 6 from block [172] team-vault routing). A full 2026-07-14 Windows run reached every Aegis block and passed **[170a–g] + [171a–g] + [172a–f] all green in-harness** (and [147] all-9-tool schemas + [168k] live audit — validating the `mcp-server.js` flush wiring); its only 13 failures were the **documented environmental subprocess/timing blocks** — [119]/[120]/[144p] MCP-stdio spawn timeouts, [121] Express `/health` startup, [125c] a perf-timing assertion — all green warm / on Linux CI, and provably unrelated to any Aegis change ([121]/[125c] touch none of it). Under resource contention (e.g. a local Supabase Docker stack + Chrome) these flake; re-run warm with a fresh Chrome + free ports, or bump `MCP_TOOL_TIMEOUT_MS` |
...
| _(Aegis — pending the eventual Aegis npm release; default-ON behavior change ships when the sinks land)_ | Aegis Step 3 — egress-boundary pipeline wiring (REDACTION_BOUNDARY_MAX_PLAN.md) | `report-processor.js` **step 3c** `classifySensitivity(report)` tags every finding `{sensitive, sensitivityReasons, localRef}` on a CLONE before the JSON write (local report keeps 100% fidelity); fail-CLOSED (`report._aegisFailClosed` on error); `ARGUS_REDACT_SENSITIVE=0` opt-out. Engine modules `secret-patterns.js` + `sensitivity-classifier.js` (Steps 1–2, prior session, +89 unit tests). New fixture `sensitive-findings.html`; **block [168]** (9 [168a–h,j]) drives the REAL pipeline (`crawlRouteCheap` + `processReport`): a `security_*` finding tagged sensitive, `redactForEgress` leaks no JWT/key/email/card substring, query-string stripped, on-disk fidelity preserved, classifier-throws fails CLOSED ([168g]). [168i] MCP `redaction` rider + Steps 4–7 egress-sink guards DEFERRED. | **970/970** |
| _(Aegis — pending the eventual Aegis npm release; default-ON behavior change for MCP consumers)_ | Aegis Step 4 — MCP egress-boundary guards (all 9 tools) | The MCP boundary is the PRIMARY agent trust boundary (OWASP LLM02). `mcp-server.js` now projects every tool response through Aegis BEFORE it crosses: findings-array tools (`argus_audit`/`argus_visual_diff`/`argus_design_audit`/`argus_pr_validate`) via `redactForEgress`; report-shaped tools (`argus_audit_full`/`argus_compare`/`argus_last_report`) via the new `redactReport` (walks `routes[].errors`/`routes[].findings`/`routes[].diffs`/`flows[].findings`/`codebase[]`, preserves metadata + summary so the [147] golden schema holds, honours `report._aegisFailClosed`); `argus_watch_snapshot`/`argus_get_context` redact findings + `deepScrub` the raw console/network arrays. Every guarded response gains an optional `redaction` rider (`{redacted,total,localReportPath,note}`). Engine change: a redacted SENSITIVE finding now carries `message` = the redact marker (a constant) so it satisfies the canonical `findingSchema`'s required `message` with ZERO golden-schema changes and every sink renders it without special-casing. `handleAudit` gained a non-client-injectable `ctx.internal` flag so `handlePrValidate`'s block/baseline/novelty/reporting logic runs on RAW findings while the MCP response is redacted. `ARGUS_REDACT_SENSITIVE=0` = byte-identical passthrough; fail-CLOSED on any projection error. Block [168] +3: [168i] the `redaction` rider safeParses + a retyped rider is rejected; [168k] a LIVE `argus_audit` on `sensitive-findings.html` (spawned server, default redaction ON) leaks no JWT/key/email/card + carries the rider; [168l] the SAME audit with `ARGUS_REDACT_SENSITIVE=0` returns the secret RAW (non-vacuity + opt-out proof). Unit suite 455→466 (+11 Step-4 helper/marker tests in `sensitivity-classifier.test.js`). Steps 5–7 (Slack/GitHub/HTML+logs) + Step 8 (vault) + Step 9 (`ARGUS_REDACT_*` env block + default-ON release note) DEFERRED. | **973/973** |
| _(Aegis — pending the eventual Aegis npm release; **default-ON behaviour change now spans every external sink**)_ | Aegis Steps 5–7 — egress-sink guards (Slack / GitHub / HTML / CI logs) | The SAME `redactForEgress` / `redactReport` projection now guards the remaining four sinks, each fail-CLOSED + `ARGUS_REDACT_SENSITIVE=0` opt-out, the pure formatters left untouched (redaction at the sink entry points): **Step 5** `dispatcher.js` `dispatchToSlack` rebinds the report to a redacted CLONE before any Slack line + adds a `🔒 N redacted` digest footer; **Step 6** `github-reporter.js` `reportToGitHub` projects the whole report via `redactReport`, `reportPrValidation` projects `result.findings` via `redactForEgress` + attaches a `redaction` rider, and `formatPrComment` renders a `> 🔒 N redacted` notice (additive — gated on `report.redaction`); **Step 7** `html-reporter.js` `generateHtmlReport(path, {external})` redacts hosted/CI renders (`shouldRedactHtml`: `ARGUS_REDACT_HTML` / default-ON under CI; the local-open render + the on-disk JSON stay RAW), and `cli/pr-validate.js` gains `safeFindingLine` (step summary + `::error`/`::warning` annotations + the printed JSON result all scrubbed). New `.env.example` `ARGUS_REDACT_*` block. **Block [169]** (5 [169a–e]): network-free reportPrValidation (no JWT/key/token in the comment + 🔒), opt-out non-vacuity, external-vs-local HTML (+ on-disk RAW fidelity), and buildStepSummary/safeFindingLine — each paired with a non-vacuity opt-out leak. New unit file `aegis-sinks.test.js` (11). Mutation-pinned. Unit 466→477 (23 files). +5 hard. Step 8 (vault) + Step 9 (release note) remain. | **978/978** |
| _(Aegis — pending the eventual Aegis npm release; OPTIONAL, default OFF — no behaviour change)_ | Aegis Step 8 — reversible re-hydration vault | OPTIONAL local tokenization (default OFF, `ARGUS_REDACT_VAULT=1`). NEW `src/utils/aegis-vault.js` — the only Aegis module doing filesystem I/O: `tokenFor`/`computeToken` mint a deterministic, information-free `AEGIS_<hmac16>` token (HMAC-SHA256, per-machine 0600 key); `storeMapping`/`loadVault` persist + read the token→secret map (`reports/.aegis-vault/<branch>.jsonl`, 0600, gitignored); `rehydrate` re-inflates a redacted artifact locally; `appendAudit` writes a SECRET-FREE provenance trail (`reports/.aegis-audit/`). `secret-patterns.js` gains a `token` scrubText mode + `setVaultTokenizer` injection seam (stays I/O-free; token mode FAILS CLOSED to mask when no vault is wired); `sensitivity-classifier.js` `redactForEgress`/`deepScrub` call `ensureVaultWired()` lazily under `ARGUS_REDACT_MODE=token`. Token mode applies to structure-preserving egress (deepScrub'd console/network/diff + benign string `value` fields); sensitive findings still drop to `REDACT_MARKER`. `scripts/rehydrate-report.mjs` + `npm run report:rehydrate -- <report.json>` re-inflate locally (shipped via `package.json` `files`). HMAC is NOT format-preserving — if shape-preserving is ever added, NIST FF1 only (FF3/FF3-1 withdrawn Feb 2025). NO harness block (filesystem I/O, Chrome-free): NEW `test/unit/aegis-vault.test.js` (15) + 3 token-mode-seam tests in `secret-patterns.test.js`. Mutation-pinned (no-vault token → mask; throwing tokenizer → mask; vault-OFF passthrough non-vacuity). Gate UNCHANGED (token mode opt-in; default mask path byte-identical). Unit 477→495 (24 files). | **978/978** |
| _(Aegis — docs only; pending the eventual Aegis npm release)_ | Aegis Step 9 — default-ON behavior-change release note | The final Aegis step (Config + docs). NEW `CHANGELOG.md` with an `[Unreleased] — Aegis` section that prominently flags the **default-ON behavior change**: external sinks (MCP / Slack / GitHub / hosted-HTML / CI logs) now show finding `type` + `route` + `severity` + a 🔒 marker, never the raw payload; the local on-disk JSON + locally-opened HTML keep 100% fidelity; fails closed; opt-out `ARGUS_REDACT_SENSITIVE=0`. **Released in v9.9.0** (maintainer-confirmed full release): the public README badge + counts move to 978/168, `server.json`/`glama.json`/`action.yml` are bumped 9.8.1→9.9.0, and the landing page gains a Security & Compliance section. REFERENCE.md needs no change (Aegis emits no new finding `type:` — grep-verified). The `.env.example` + CLAUDE env-table `ARGUS_REDACT_*` blocks landed across Steps 4–8. **Aegis (Steps 1–9) COMPLETE.** | **978/978** |
| _(Aegis-for-Teams — released in v10.0.0; opt-in, default byte-identical — no behaviour change)_ | Aegis-for-Teams — engine `policy` param (AEGIS_FOR_TEAMS_PLAN.md §4.2/§9) | The runtime "apply" mechanism the Enterprise governance seam wraps. NEW `src/utils/redaction-policy.js` (pure, zero-dep, zero-I/O, no engine imports): `resolvePolicy` (fail-CLOSED to `STRICT_DEFAULT` on missing/malformed/throwing) / `policyOpts` / `policyFromEnv` (`ARGUS_REDACT_POLICY` inline JSON, cached; SET-but-malformed ⇒ strict floor) / `effectivePolicyOpts` (raw `opts.policy` > env > none). A valid §4.2 policy: NARROWS the rule set (`rules.secret.all` / `rules.pii.<name>:false`), ADDS org-custom secret rules (`rules.secret.extra`, compiled fail-safe `/g`, additive), WIDENS the sensitive-type set (union, `prefix_*` glob), and NARROWS the egress allowlist (intersection with `EGRESS_ALLOWLIST` — never widens); `floor.failOpen` FORCED false. The entropy layer + category/body-field catch-alls are NOT policy-toggleable (the no-leak floor). Threaded via opts through `findSensitiveSpans`/`scrubText` (ruleFilter/extraSecretRules), `classifyFinding`/`isSensitiveType` (sensitiveTypes union), `projectFinding`/`redactForEgress` (allowSet), `deepScrub`/`redactReport`/`summarizeRedaction`/`buildRedactionRider`. NO policy ⇒ byte-identical to v9.9.0. **Block [170]** (7 [170a–g], Chrome-free/pure): toggle flows through the real projector, sensitiveTypes widens-only, egress allowlist narrows-only (a policy can't add a leaky field), malformed/throwing policy fails CLOSED, no-leak floor holds with secret rules OFF, no-policy byte-identical, `ARGUS_REDACT_POLICY` env valid+malformed. NEW `test/unit/redaction-policy.test.js` (32). Unit 495→527. +7 hard. | **985/985** |
| _(Aegis-for-Teams — released in v10.0.0; opt-in, INERT without a gov token — no behaviour change by default)_ | Aegis-for-Teams — governance seam (AEGIS_FOR_TEAMS_PLAN.md §9 — the opt-in network seam) | The thin OPT-IN network wrapper over the `policy` param: fetch the org's Ed25519-**signed** policy → **verify** it → apply → TTL-cache → **fail CLOSED** to the strict floor on any error → post SECRET-FREE aggregates. NEW `src/utils/governance-seam.js` (node:crypto + global fetch — NO new dep): `ensureGovernancePolicy` (async, best-effort, NEVER throws) / `verifySignedPolicy` (Ed25519 via `crypto.verify(null,…)` against pinned `ARGUS_REDACT_POLICY_PUBKEY`; throws on bad-sig/wrong-key/missing-pubkey) / `canonicalPolicyBytes` (key-sorted, cloud-signer interop) / `postRedactionAggregate` (`summarizeRedaction` byReason label counts → `ARGUS_REDACT_AUDIT_URL`; every label scrubbed) / `governanceActive` / `governancePolicyVersion`. `redaction-policy.js` gains the pure `setGovernanceSource`/`governanceOptOutClamped` injection seam (I/O stays in governance-seam.js); `effectivePolicyOpts` precedence `opts.policy > governance > env`; `sensitivity-classifier.js` clamps the `ARGUS_REDACT_SENSITIVE=0` opt-out under a gov token (§4.3). Wired into `report-processor.js` `processReport` (fetch at start, secret-free aggregate after step 3c — both inert without a token). **INERT without `ARGUS_GOV_TOKEN` ⇒ byte-identical to v9.9.0** (open-core: no phone-home, no new dep, no perf/privacy cost). **Block [171]** (7 [171a–g]): [171a] signed verifies + email-OFF toggle applies + TAMPERED body rejected→floor, [171b] wrong-key rejected, [171c] unreachable/malformed/HTTP-error fail CLOSED, [171d] SECRET-FREE aggregate (tainted label + runRef scrubbed, counts kept), [171e] gov token CLAMPS opt-out + no-token inert/byte-identical, [171f] TTL cache, [171g] LIVE HTTP fetch→verify→apply→aggregate over a spawned Ed25519 mock control plane. NEW `test/unit/governance-seam.test.js` (21). The §9 team-vault routing that was deferred here landed in the next row. Unit 527→548. +7 hard. | **992/992** |
| _(Aegis-for-Teams — released in v10.0.0; opt-in, INERT without both a gov token + a vault URL — no behaviour change by default)_ | Aegis-for-Teams — §9 team-vault routing (`mode=token`→central vault, the last T3 sub-item) | Route the reversible token→secret mapping to the org's CENTRAL vault (re-hydratable via the T1 RBAC `Reveal ▸` flow) instead of the local 0600 file, so a token minted on a laptop/CI is re-hydratable org-wide. NEW `src/utils/team-vault.js` (global fetch only — NO new dep): `teamVaultActive` (gov token + `ARGUS_REDACT_VAULT_URL`) / `teamTokenFor` (SYNC `computeToken` HMAC + buffer, deduped, cap) / `ensureTokenVaultWired` (unified `token` mode entry — team vault **>** local vault, §4.3) / `flushTeamVault` (async best-effort POST `{mappings:[{token,secret}],artifactRef}` bearer-authed — the SECRET goes ONLY to the endpoint, the info-free token crosses every other sink) / `pendingTeamVaultCount`. **FAIL-CLOSED:** a throwing/HTTP-error flush never throws + RETAINS the buffer (the emitted token is information-free — a failed flush leaks nothing); token mode with the seam OFF falls back to the local vault / mask. `sensitivity-classifier.js` two `token` mode ensure sites → `ensureTokenVaultWired`; `mcp-server.js` `await flushTeamVault()` after each tool response. **INERT unless BOTH `ARGUS_GOV_TOKEN` + `ARGUS_REDACT_VAULT_URL` set ⇒ byte-identical to v9.9.0.** Cloud half: `POST /api/gov/vault` (gov-token bearer) + `ingestVaultMappings`/`sanitizeVaultMappings` reusing the T1 `flushVaultMappings` (AES-256-GCM at rest), `proof:gov-vault` **13/13** against the REAL OSS engine (live flush→encrypt-at-rest→re-hydration round-trip, entitlement guard-off, cross-tenant, RLS deny-all, revocation). **Block [172]** (6 [172a–f]): opt-in activation, mint+buffer+no-leak via the real `deepScrub`, LIVE flush to a spawned localhost sink (secret to the authorized endpoint ONLY, buffer cleared), fail-closed (throw + HTTP-error), opt-in default-identical, team>local precedence — deterministically proven 6/6. NEW `test/unit/team-vault.test.js` (14). Unit 548→562. +6 hard. | **998/998** |
